...
import { createPvt, getPvtJobCategories,removePvtJobCategory } from '../../actions/privateJobCategory';


const PvtJobCategory=()=>{
    ...
    const { name, error, success, privateJobCategories, removed,reload } = values;
    ...
    const deleteJobCategory = slug => {
        ...
        removePvtJobCategory(slug, token).then(data => {
            if (data.error) {
                console.log(data.error);
            } else {
    ...
    const clickSubmit = e => {
        ...
        createPvt({ name }, token).then(data => {
            if (data.error) {
