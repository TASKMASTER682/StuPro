...
import { createPvt, getPvtJobTags, removePvtJobTag } from '../../actions/privateJobTag'
...
const PvtJobTag=()=>{
    ...
    const { name, error, success, privateJobTags, removed, reload } = values;
    ...
    const deleteJobTag = slug => {
        ...
        removePvtJobTag(slug, token).then(data => {
            if (data.error) {
                console.log(data.error);
            } else {
